...
function buildContainer() {
    ...
    const el = `<div id='${containerId}'>
                    <p>本文作者：${author}</p>
                    <p>本文链接：<a rel="nofollow" href="${href}">${href}</a></p>
                </div>`
...
const buildLicense = (enableLicense, licenseName, licenseLink) => {
    ...
    const agreeLink = licenseLink ? `<a rel="nofollow" href="${licenseLink}">许可协议</a>` : '许可协议'
    const el = `<p>版权声明：本作品采用${agreement}${agreeLink}进行许可。</p>`
